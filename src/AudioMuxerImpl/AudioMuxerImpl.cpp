...
static bool open_audio(AVFormatContext* /*oc*/, AVCodec* codec, OutputStream* ost,
...
static AVFrame* get_audio_frame(OutputStream* ost, int16_t* data, int /*channel*/, int totalSamples,
                                int* pConsumeSamples) {
...
static bool write_audio_frame(AVFormatContext* oc, OutputStream* ost, AVFrame* frame) {
  ...
  if (frame) {
    ...
    ret = swr_convert(ost->swr_ctx, ost->internalFrame->data, dst_nb_samples,
...
bool AudioMuxerImpl::init(std::string& outputPath, int channel, int sampleRate) {
  int ret;
  AVDictionary* opt = NULL;
  ...
  if (!(fmt->flags & AVFMT_NOFILE)) {
    ...
    if (ret < 0) {
      fprintf(stderr, "Could not open '%s': %s\n", filename.c_str(), av_err2string(ret));
      return 1;
    }
  }
...
int AudioMuxerImpl::putData(int16_t* data, int numSamples) {
  int samples = 0;

  while (samples < numSamples) {
    int comsumeSamples = 0;
    auto frame = get_audio_frame(&audio_st, data + samples * channel, channel, numSamples - samples,
                                 &comsumeSamples);
    if (frame == nullptr) {
      break;
    }
...
AudioMuxerImpl::~AudioMuxerImpl() {
  remove(filename.c_str());
}

std::string AudioMuxerImpl::getFilename() {
  return filename;
}